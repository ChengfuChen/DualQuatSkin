...
var AniReader = (function () {
    ...
    cls.prototype.loadObj = function(url, callBack, index){
    	var req = new XMLHttpRequest();
		if(url.indexOf(".asf")!=-1) 
		        req.onreadystatechange = function () { cls.prototype.processLoadASF(req,callBack);};
		else if(url.indexOf(".amc")!=-1)
				req.onreadystatechange = function () { cls.prototype.processLoadAMC(req,callBack,url,index);};
    ...
    cls.prototype.parseASF = function(text){
    	...
    	for(var lineIndex in args){
    		...
    		if(args[lineIndex].split(' ').length==2){//two words single line
    			var params = args[lineIndex].split(' ');
    			params[0] = params[0].split(':')[1];
    			result[params[0]] = params[1];
    		}
    		else{//multi lines
    			var lines = args[lineIndex].match(/[\w].*/g);
    			...
    			if(!cls.prototype.hasTag(lines)){//Don't have tag like "begin","end"
    				...
    				var childDoc = args[lineIndex].match(/begin[^:]*?end/g);
    ...
    cls.prototype.getChild=function (data){
    		...
    		if(paramNms[0]=="length")
    			paramNms[0]="_length";//I cannot use "length" since it is system reserved name
...
//    };
    ...
    cls.prototype.parseAMC = function(text,asfData)
    {
    	var frameNumber = [];
    	animData = [];
    	var args = text.split('\n');
    	...
    	for(var lineIndex = 3;lineIndex<maxLength;lineIndex++){
    		var lines = args[lineIndex].split(' '); 
    		if(lines.length==1){//frame number
    			...
    			else
    			{
    				...
    				for(var boneOrder in asfData["bonedata"])
    				{
    					...
    					if(name.substr(0,lines[0].length) == lines[0])
    					{
    						...
    						for( var dofCount =0;dofCount<dof.length;dofCount++)
    						{
    							...
    							{
    							case "rx":
    								dofs.rx = lines[dofCount+1];
    								break;
    							case "ry":
    								dofs.ry = lines[dofCount+1];
    								break;
    							case "rz":
    								dofs.rz = lines[dofCount+1];
    								break;
    							}			
    						}
    					}
    				...
    				animData.push({jointName:lines[0],dof:dofs});
    			}
    		}
    	}
    	return animData;
...
//    
//    cls.prototype.processLoadASF = function(req,callBack)
//    {
//    	var result;
//        if (req.readyState == 4) {
//        
...
//    		
//        	result =cls.prototype.parseASF(req.responseText);
//        	this._asfData = result;
//        	callBack.call(this,this._asfData);
//        	
//        }
//    };
//    cls.prototype.processLoadAMC = function(req,callBack,url,index)
//	{
//    	var result;
//		if (req.readyState == 4) {
//			
//			result = cls.prototype.parseAMC(req.responseText,this._asfData);
//			this._amcData = this.wrapDataIntoFrames(result);
//			//cls.prototype.printAnimData();
//			callBack.call(this,this._amcData,url,index);
//		}
//	};
    cls.prototype.wrapDataIntoFrames = function(data){
    	var frames = [];
    	var pose;
    	for(var i=0;i<data.length;i++){
